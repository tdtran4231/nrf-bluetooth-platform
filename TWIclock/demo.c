...
static app_twi_t m_app_twi = { .twi = NRF_DRV_TWI_INSTANCE(0) };
...
// AT24C32 write address is 0xAE, read address is 0xAF 
static uint8_t epromaddr[2] = { 0x0f, 0x20 };
static uint8_t buffer[1];
static bool have_read = false; 
static void after_read_memory(ret_code_t a, void * b) {
  gpioOn(19);
  have_read = true;
  // gpioOff(18);
  if (buffer[0]!=0x31) gpioOn(21);
  }
static void read_memory() {
  static app_twi_transfer_t const transfers[] = {
    { .p_data=epromaddr, .length=2, .operation=0xAE, .flags=APP_TWI_NO_STOP },
    { .p_data=buffer, .length=0, .operation=0xAF, .flags=0 }
    };  
  static app_twi_transaction_t const transaction = {
    .callback=after_read_memory, .p_user_data=NULL, 
    .p_transfers=transfers, .number_of_transfers=2 }; 
  if (have_read) return;
  buffer[0] = 0x31;
  gpioOn(18);
  app_twi_schedule(&m_app_twi,&transaction);
  }

static void twi_config() {
  uint32_t err_code;
  nrf_drv_twi_config_t const config = {
    .scl                = ARDUINO_SCL_PIN,   // see PCA10028.h 
    .sda                = ARDUINO_SDA_PIN,
    .frequency          = NRF_TWI_FREQ_100K,
    .interrupt_priority = APP_IRQ_PRIORITY_LOW
    };
  APP_TWI_INIT(&m_app_twi,&config,5,err_code);
  err_code += 0;
  }
